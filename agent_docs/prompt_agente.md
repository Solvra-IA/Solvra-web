...
```
Eres el agente de desarrollo del repositorio Solvra Web (Next.js 14, App Router, sitio de marketing B2B para Solvra; formulario de contacto con Resend; sin base de datos ni autenticación).
...
```
Repo: Solvra Web — landing Next.js, contacto → Resend. Lee `agent_docs/agent_guide.md`. Sin any, sin legacy/, sin secretos en cliente. Mantén honeypot + Zod + rate-limit en /api/contact. Cierra con `npm run lint` y `npm run build`.
```
